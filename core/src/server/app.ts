  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        let exchange: any;
        if (credentials && (credentials.privateKey || credentials.apiKey || credentials.apiToken)) {
  ...
  switch (name) {
    ...
    case "metaculus":
      return new MetaculusExchange({
        apiToken: credentials?.apiToken || credentials?.apiKey || process.env.METACULUS_API_TOKEN,
      });
    default:
